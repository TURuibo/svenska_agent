...
**例句规则 (查词时聊天里必带，sense-aware)：** 查**单个词/词组**时，聊天回复**一定要附例句**，
并**按不同义项分组**给（和文件里的 sense-aware 计数一致）：
- 多义项 → **每个义项至少 1 个例句**，每行前面标 **中文义项标签**（如 `· 教:`、`· 打/击:`）。
- **partikelverb（slå på/upp…）、反身用法（slå sig）、固定搭配各算一个独立义项**，要单独成行并标签，
  例如 `· 开/关 (slå på/av):`、`· 查词 (slå upp):`、`· 磕到自己 (slå sig):`。这是首选样式（用户偏好）。
- 单一义项（或义项相近）→ 直接给 **2–3 个例句**，展示典型用法。
- 例句维持 A1–A2 难度、🇸🇪 原文 + 🇨🇳 翻译，关键词可 **加粗**。这不与"聊天框精简"冲突——例句是查词的核心交付，
  其余细节（完整变形表、搭配、词族链接、用法陷阱）仍只放在 md 文件里。
- 词条已存在但例句单薄/缺义项时，**enrich 文件并在聊天用同样的分义项格式补齐**（见 slå 的处理）。
- 句子/整段/图片输入不强制（那类输入聊天给整体理解，逐词例句进文件即可）。

> 黄金样式参考（用户点名喜欢，照此办）：
> ```
> 📌 例句（按义项）：
>  · 打/击: 🇸🇪 Han slog mig på axeln. — 🇨🇳 他拍了下我肩膀。
>  · 打破记录: 🇸🇪 Zlatan slog rekord. — 🇨🇳 Zlatan 打破了记录。
>  · 开/关 (slå på/av): 🇸🇪 Kan du slå på lampan? — 🇨🇳 你能把灯打开吗？
>  · 查词 (slå upp): 🇸🇪 Jag slår upp ordet i ordboken. — 🇨🇳 我在词典里查这个词。
>  · 磕到自己 (slå sig): 🇸🇪 Barnet slog sig i huvudet. — 🇨🇳 孩子磕到了头。
> ```
