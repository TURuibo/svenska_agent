...
1. **重建站点数据（best-effort）**：
   ...
   - 该脚本有纯 PowerShell 回退，无 node 也能跑；若失败不要中断——GitHub Action 会在 push 后重建 `site/kb-data.js`。

2. **只暂存真实内容路径**（存在才加；绝不加 `inbox/`、`traces/`、`settings.local.json`）：
   `powershell -NoProfile -Command "git add -- knowledge_base review/schedule.md profile site/kb-data.js .gitattributes"`
