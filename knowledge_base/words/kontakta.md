---
type: word
lemma: "kontakta"
ordklass: verb
verbgrupp: "1"
genus: ""
cefr: "A2"
zh: 联系；取得联系
en: to contact; to get in touch with
synonyms: []
antonyms: []
family: []
topics: [topic-skola-och-utbildning, topic-social-kontakt]
sentences: [sent-om-du-inte-kan-komma, sent-om-ingen-tid-passar]
known: false
created: "2026-06-24"
reviewed: ""
review_count: 0
ease: 2.5
interval: 0
---

# kontakta — verb (grupp 1)

📖 中文：联系；取得联系 · English: to contact; to get in touch with
发音提示：kon-TAK-ta

## 语法变形 (Forms)

| form | Swedish |
|------|---------|
| infinitiv | kontakta |
| presens | kontaktar |
| preteritum | kontaktade |
| supinum | kontaktat |
| imperativ | kontakta |

## 词组搭配 (Collocations)

- `kontakta oss` — 联系我们
- `kontakta snarast` — 尽快联系 → see [[snarast-möjligt]]
- `kontakta klassläraren` — 联系班主任 → see [[klasslärare]]
- `kontakta myndigheten` — 联系机关

## 例句 (Sentences)

- 🇸🇪 Om du inte kan komma, kontakta oss [[snarast-möjligt|snarast möjligt]]. — 🇨🇳 如果您无法前来，请尽快联系我们。
- 🇸🇪 Du kan kontakta skolan via e-post eller telefon. — 🇨🇳 您可以通过电子邮件或电话联系学校。
- 🇸🇪 Jag kontaktade läkaren för att boka en tid. — 🇨🇳 我联系了医生预约了时间。
- 🇸🇪 Om ingen tid passar, kontakta mig på e-post. — 🇨🇳 如果没有合适的时间，请通过邮件联系我。

## 词族 / 同义 / 反义 (Relations)

- 词族: [[kontakt]] (en; contact)
- 同义词: [[ringa]] (打电话), [[skriva-till]] (写信)
- 反义词: —
- 主题: [[topic-skola-och-utbildning]], [[topic-social-kontakt]]

## 用法提示 (Usage Notes)

- Mer formellt än "ringa" eller "skriva till". Vanligt i officiella brev och e-post.
- Används gärna när man vill vara neutral om kommunikationsmetoden (telefon, e-post, etc.).
