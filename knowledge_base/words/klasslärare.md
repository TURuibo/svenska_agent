---
type: word
lemma: "klasslärare"
ordklass: substantiv
verbgrupp: ""
genus: en
cefr: "A2"
zh: 班主任；班级老师
en: class teacher; homeroom teacher
synonyms: []
antonyms: []
family: []
topics: [topic-skola-och-utbildning, topic-yrken]
sentences: []
known: false
created: "2026-06-24"
reviewed: ""
review_count: 0
ease: 2.5
interval: 0
---

# klasslärare — substantiv (en)

📖 中文：班主任；班级老师 · English: class teacher; homeroom teacher
发音提示：KLASS-lä-ra-re

## 语法变形 (Forms)

| form | Swedish |
|------|---------|
| obestämd singular | en klasslärare |
| bestämd singular | klassläraren |
| obestämd plural | klasslärare |
| bestämd plural | klasslärarna |

## 词组搭配 (Collocations)

- `kontakta klassläraren` — 联系班主任 → see [[kontakta]]
- `prata med klassläraren` — 和班主任谈话
- `klasslärare i klass 4B` — 4B班班主任

## 例句 (Sentences)

- 🇸🇪 Fatima Andersson är klasslärare i klass 4B. — 🇨🇳 Fatima Andersson是4B班的班主任。
- 🇸🇪 Du kan kontakta klassläraren om du har frågor. — 🇨🇳 如果您有问题，可以联系班主任。
- 🇸🇪 Barnets klasslärare bjöd in föräldrarna till ett möte. — 🇨🇳 孩子的班主任邀请家长来开会。
- 🇸🇪 Lena Ström är klasslärare i klass 2C på Björkbacken skola. — 🇨🇳 Lena Ström是Björkbacken学校2C班的班主任。

## 词族 / 同义 / 反义 (Relations)

- 词族: [[klass]] + [[lärare]] (teacher)
- 同义词: [[lärare]]
- 反义词: —
- 主题: [[topic-skola-och-utbildning]], [[topic-yrken]]

## 用法提示 (Usage Notes)

- Sammansatt ord: **klass** (班级) + **lärare** (老师).
- En klasslärare har ansvar för en specifik klass — ungefär som en "homeroom teacher" i USA eller en "班主任" i Kina.
