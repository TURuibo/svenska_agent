---
type: topic
slug: topic-källsortering
title: Källsortering & Avfallshantering
zh: 垃圾分类与废物处理
en: Waste sorting & waste management
related_topics: [topic-miljö-och-klimat, topic-hemmet]
words:
  - källsortering
  - avfall
  - matavfall
  - restavfall
  - behållare
  - påse
  - kaffesump
  - äggskal
  - wellpapp
  - blöja
  - återvinningsstation
  - batteri
  - kemikalie
  - återvinning
  - hushåll
  - soptunna
  - miljöstation
  - sopbil
  - biogas
  - kompost
  - naturresurs
  - förbränna
  - spara
  - kasta
phrases:
  - källsortera-hemma
  - bind-ihop
  - vika-ihop
  - lämna-vid
  - farligt-avfall
  - källsortera-sina-sopor
  - lägger-man-i
  - tas-till
sources:
  - source-2026-06-25-instruktion-kallsortering
  - source-2026-06-27-faktatext-atervinning
created: "2026-06-25"
---

# Källsortering & Avfallshantering — 垃圾分类与废物处理

## 核心概念 (Core Concepts)

| Avfallstyp | 中文 | 容器颜色 | 处理方式 |
|------------|------|----------|----------|
| [[matavfall]] | 厨余垃圾 | 棕色袋 | 制沼气/堆肥 |
| [[restavfall]] | 普通垃圾 | 灰色袋 | 焚烧发电 |
| [[farligt-avfall]] | 危险废品 | 专用 | 送回收站 |
| 纸板/[[wellpapp]] | 纸类 | 专用 | 纸张回收 |

## 词汇 (Words)

### 垃圾类型
- [[källsortering]] — 垃圾分类（总称）
- [[avfall]] — 废物、垃圾
- [[matavfall]] — 厨余垃圾
- [[restavfall]] — 普通垃圾

### 容器与工具
- [[behållare]] — 容器（通用）
- [[påse]] — 袋子
- [[soptunna]] — 垃圾桶
- [[återvinningsstation]] — 回收站
- [[miljöstation]] — 环保站（危险废物）

### 处理方式
- [[återvinning]] — 回收利用
- [[biogas]] — 沼气（厨余再利用）
- [[kompost]] — 堆肥（厨余再利用）
- [[förbränna]] — 焚烧
- [[spara]] — 节省（资源）
- [[naturresurs]] — 自然资源

### 交通运输
- [[sopbil]] — 垃圾车

### 行为动词
- [[kasta]] — 扔/丢弃

### 具体废弃物
- [[kaffesump]] — 咖啡渣
- [[äggskal]] — 蛋壳
- [[wellpapp]] — 瓦楞纸
- [[blöja]] — 尿布
- [[batteri]] — 电池（危险废品）
- [[kemikalie]] — 化学品（危险废品）

## 词组 (Phrases)

- [[källsortera-hemma]] — 在家进行垃圾分类
- [[källsortera-sina-sopor]] — 分类处理自己的垃圾
- [[lägger-man-i]] — 人们把……放进（泛指句型）
- [[tas-till]] — 被送到……
- [[bind-ihop]] — 把袋子扎好
- [[vika-ihop]] — 把纸板折叠好
- [[lämna-vid]] — 送到…处
- [[farligt-avfall]] — 危险废品

## 相关主题 (Related Topics)

- [[topic-miljö-och-klimat]] — 环境与气候
- [[topic-hemmet]] — 家居
