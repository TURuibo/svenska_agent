---
type: phrase
phrase: "dömas till fängelse"
category: fast-uttryck
register: formellt
frequency: 3
zh: "被判处监禁"
en: "to be sentenced to prison"
literal: "to be judged to prison"
head_words: [döma, fängelse]
grammar: [grammar-passiv-med-s]
related: [bli-dömd-till, livstids-fängelse]
topics: [topic-krig-och-konflikt]
sentences:
  - sent-han-doms-till-livstids-fangelse
  - sent-mannen-som-kastade-handgranaten-doms-till-fangelse
  - sent-nu-döms-mamman-till-sex-års
known: false
created: "2026-06-24"
reviewed: ""
review_count: 0
ease: 2.5
interval: 0
---

# dömas till fängelse

🗣️ 类型: fast-uttryck · 语域: formellt · 常用度: 3/5
🇨🇳 被判处监禁 · 🇬🇧 to be sentenced to prison
📝 字面: to be judged to prison

## 用法说明 (Usage)

- s-passiv av "döma" (att döma = to sentence/judge)
- "döms" = presens passiv; "dömdes" = preteritum passiv
- Ofta följt av tidsangivelse: "döms till 14 år i fängelse"

## 短语中的语法 (Grammar inside)

- → [[grammar-passiv-med-s]]

## 变体 & 相关 (Variations & Related)

- 🔄 bli dömd till fängelse (perifrastisk passiv)
- 🔗 [[bli-dömd-till]]
- 🔗 [[livstids-fängelse]]

## 例句 / 对话 (Examples)

- [[sent-han-doms-till-livstids-fangelse]] 🇸🇪 Han döms till livstids fängelse. 🇨🇳 他被判处无期徒刑。
- [[sent-mannen-som-kastade-handgranaten-doms-till-fangelse]] 🇸🇪 Mannen som kastade handgranaten döms till fängelse i nästan 14 år. 🇨🇳 投掷手榴弹的男子被判处近14年有期徒刑。
- 🇸🇪 Han dömdes till tre års fängelse för bedrägeriet. 🇨🇳 他因欺诈罪被判处三年有期徒刑。
- [[sent-nu-döms-mamman-till-sex-års]] 🇸🇪 Nu döms mamman till sex års fängelse. 🇨🇳 现在这位母亲被判处六年监禁。
