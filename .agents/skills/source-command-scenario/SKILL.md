---
name: "source-command-scenario"
description: "Generate a Swedish practice scenario (dialogue/text/narrative) into inbox/ for review then /import"
---

# source-command-scenario

Use this skill when the user runs `/scenario` or asks to run the migrated Claude Code command `scenario` in Codex.
...
For example, topic "fråga efter vägen" → slug `fraga-efter-vagen` → path `inbox/scenario-<DATE>-fraga-efter-vagen.md`.
...
📁 已写入: inbox/scenario-<DATE>-<slug>.md

⏭ 审阅后运行 /import 录入（或 /import scenario-<DATE>-<slug>.md）
