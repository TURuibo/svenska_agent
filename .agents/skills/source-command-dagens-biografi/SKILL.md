---
name: "source-command-dagens-biografi"
description: "生成/抓取一篇 SFI 风格的瑞典语人物传记（lättläst）写入 inbox/（人读正文 + svensk-export 导入块），供 /import 入库。仿 Astrid Lindgren / Zlatan 那类传记文章。"
---

# source-command-dagens-biografi

Use this skill when the user runs `/dagens-biografi` or asks to run the migrated Claude Code command `dagens-biografi` in Codex.
