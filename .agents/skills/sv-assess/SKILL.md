...
Maintain `profile/level.md`: the single source of truth for what Ruibo already knows, so Codex can
...
Before a full dictionary entry, Codex checks `profile/level.md` 已掌握 + the note's `known` flag.
